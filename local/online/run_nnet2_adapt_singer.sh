...
ali=exp/vocal/svtri3b_svtri1_1k_5k_ali_spk_exp3
if [ $stage -le -1 ]; then
  trainset=data/vocal_spk/train_clean
  lm=data/lang_lyric
...
EOF
  ...
  dir=exp/vocal/nnet2_online/nnet_a_singer
  trainfeats=exp/vocal/nnet2_online/activations_train_from_speech_to_vocal_singer
...
if [ $stage -le 0 ]; then
  ...
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj $numjob \
     data/vocal_spk/train_clean $srcdir $trainfeats
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal_spk/test_clean ${dir}_online/decode || exit 1;
  wait
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
     --per-utt true \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal_spk/test_clean ${dir}_online/decode_utt || exit 1;
  wait
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/get_egs.sh --cmd "$train_cmd" --num-jobs-nnet 4 \
    data/vocal_spk/train_clean $ali ${dir}_online ${dir}_combined
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal_spk/test_clean ${dir}_combined_online/decode &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    --per-utt true exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal_spk/test_clean ${dir}_combined_online/decode_per_utt || exit 1;
  wait
fi
