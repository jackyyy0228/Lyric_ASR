...
train_hires=data/sp_exp/train_0sp_hires
train_sp=data/sp_exp/train_0sp
ivectordir=exp/nnet3/sp_exp/ivectors_train_0sp_hires
...
if [ $stage -le 2 ] && [  -f $train_hires/feats.scp ]; then
  echo "$0: data/$train_hires/feats.scp already exists."
...
if [ $stage -le 2 ]; then
  ...
  utils/copy_data_dir.sh $train_set $train_hires
...
if [ $stage -le 8 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    $train_set data/lang_lyric_exten $gmm_dir $ali_dir
fi
