...
EOF
  ...
  num_threads=1
  minibatch_size=512
  dir=exp/vocal/nnet2_online/gpu_nnet_utt_exp1
  trainfeats=exp/vocal/nnet2_online/activations_train_utt_from_speech_to_vocal
  testfeats=exp/vocal/nnet2_online/activations_test_utt_from_speech_to_vocal
  ...
  dir=exp/vocal/nnet2_online/threads_nnet_utt_exp1
  trainfeats=exp/vocal/nnet2_online/activations_train_utt_from_speech_to_vocal
  testfeats=exp/vocal/nnet2_online/activations_test_utt_from_speech_to_vocal
...
if [ $stage -le 0 ]; then
  ...
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj $numjob \
     data/vocal/uttadp_train_clean $srcdir $trainfeats
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj $numjob \
     data/vocal/uttadp_test_clean $srcdir $testfeats
fi
...
if [ $stage -le 1 ]; then
  ...
  local/nnet2/retrain_fast3.sh --stage $train_stage \
    ...
    --initial-learning-rate 0.02 --final-learning-rate 0.004 \
     $trainfeats/data $testfeats/data data/lang_lyric exp/vocal/tri3b_ali_all_clean_utt $dir 
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/genre/new_gat/tri3b_utt/graph_tgsmall data/vocal/uttadp_test_clean ${dir}_online/decode || exit 1;
  wait
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
     --per-utt true \
    exp/genre/new_gat/tri3b_utt/graph_tgsmall data/vocal/uttadp_test_clean ${dir}_online/decode_utt || exit 1;
  wait
fi
...
if [ $stage -le 6 ]; then
  ...
  local/online/nnet2/get_egs3.sh --cmd "$train_cmd" --num-jobs-nnet 4 \
    data/vocal/uttadp_train_clean data/vocal/uttadp_test_clean exp/vocal/tri3b_ali_all_clean_utt ${dir}_online ${dir}_combined
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/genre/new_gat/tri3b_utt/graph_tgsmall data/vocal/uttadp_test_clean ${dir}_combined_online/decode &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    --per-utt true \
    exp/genre/new_gat/tri3b_utt/graph_tgsmall data/vocal/uttadp_test_clean ${dir}_combined_online/decode_per_utt || exit 1;
  wait
fi
