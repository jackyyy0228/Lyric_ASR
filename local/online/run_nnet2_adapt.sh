...
if [ $stage -le 1 ]; then
  ...
  steps/nnet2/retrain_fast.sh --stage $train_stage \
    ...
    --initial-learning-rate 0.01 --final-learning-rate 0.002 \
     $trainfeats/data data/lang_lyric exp/vocal/svtri3b_svtri1_1k_5k_ali_exp3 $dir 
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/get_egs.sh --cmd "$train_cmd" --num-jobs-nnet 4 \
    data/vocal/ori_train_clean exp/vocal/svtri3b_svtri1_1k_5k_ali_exp3 ${dir}_online ${dir}_combined
fi
