...
if [ ! -d $s1 ]; then 
  ...
  for f in utt2spk spk2utt spk2warp feats.scp text wav.scp cmvn.scp spk2gender \
    vad.scp segments reco2file_and_channel utt2lang level2spk spk2level; do
...
for n in `seq $numsplit`; do
   ...
   spk2levels="$spk2levels $data/split$numsplit/$n/spk2level"
done

if $split_per_spk; then
  utt2spk_opt="--utt2spk=$data/utt2spk"
  spk2level_opt="--utt2spk=$data/spk2level"
else
  utt2spk_opt=
  spk2level_opt=
fi
...
utils/split_scp.pl $spk2level_opt $data/spk2level $spk2levels || exit 1

for n in `seq $numsplit`; do
  dsn=$data/split$numsplit/$n
  utils/utt2spk_to_spk2utt.pl $dsn/utt2spk > $dsn/spk2utt || exit 1;
  utils/utt2spk_to_spk2utt.pl $dsn/spk2level > $dsn/level2spk || exit 1;
done
