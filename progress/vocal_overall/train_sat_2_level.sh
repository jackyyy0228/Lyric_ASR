  ...
  if echo $fmllr_iters | grep -w $x >/dev/null; then
    if [ $stage -le $x ]; then
      ...
      $cmd JOB=1:$nj $dir/log/fmllr.$x.JOB.log \
        ...
        "$feats" ark:- ark,t:$dir/tmp_trans.JOB || exit 1;
      $cmd JOB=1:$nj $dir/log/level_fmllr.$x.JOB.log \
        ali-to-post "ark:gunzip -c $dir/ali.JOB.gz|" ark:-  \| \
        weight-silence-post $silence_weight $silphonelist $dir/$x.mdl ark:- ark:- \| \
        gmm-est-fmllr --fmllr-update-type=$fmllr_update_type \
        --spk2utt=ark:$sdata/JOB/level2spk $dir/$x.mdl \
        "$feats" ark:- ark,t:$dir/tmp_raw_trans2.JOB || exit 1;
      for n in `seq $nj`; do
        python dup_trans.py --level2spk=$sdata/$n/level2spk --in-trans=$dir/tmp_raw_trans2.$n \
          --out-trans=$dir/tmp_trans2.$n
      done
      for n in `seq $nj`; do
        ! ( compose-transforms --b-is-affine=true \
          ark,t:$dir/tmp_trans2.$n ark,t:$dir/tmp_trans.$n ark:$dir/tmp_composed_trans.$n &&
          rm $dir/tmp_trans.$n && rm $dir/tmp_raw_trans2.$n &&\
          rm $dir/tmp_trans2.$n) 2>$dir/log/tmp_compose_transforms.$x.log \
          && echo "$0: Error composing transforms for tmp" && exit 1;
        ! ( compose-transforms --b-is-affine=true \
          ark:$dir/tmp_composed_trans.$n ark:$cur_trans_dir/trans.$n ark:$dir/composed_trans.$n \
          && mv $dir/composed_trans.$n $dir/trans.$n && \
          rm $dir/tmp_composed_trans.$n ) 2>$dir/log/compose_transforms.$x.log \
