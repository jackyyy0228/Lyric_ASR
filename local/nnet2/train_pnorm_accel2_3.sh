...
# Check some files.
for f in $data/feats.scp $testdata/feats.scp $lang/L.fst $alidir/final.mdl $alidir/tree; do
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  echo "$0: calling get_egs2_3.sh"            
  local/nnet2/get_egs2_3.sh $egs_opts "${extra_opts[@]}"  --io-opts "$io_opts" \
    --postdir "$postdir" --samples-per-iter $samples_per_iter --stage $get_egs_stage \
    --cmd "$cmd" $egs_opts \
    $data $testdata $alidir $dir/egs || exit 1;
fi
