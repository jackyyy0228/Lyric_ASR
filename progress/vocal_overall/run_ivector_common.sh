...
ubm=exp/paper/nnet_ivec/diag_ubm
extractor=exp/paper/nnet_ivec/extractor
ivec=exp/paper/nnet_ivec/ivectors
...
if [ $stage -le 7 ]; then
  ...
  for group in train_clean test_clean; do
    steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 data/paper/${group}_utt data/paper/${group}_utt_max2
    
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $numjob \
      data/paper/${group}_utt_max2 $extractor ${ivec}_${group} || exit 1;
  done
fi
