...
if [ $# != 5 ]; then
  echo "Usage: steps/nnet2/get_egs3.sh [opts] <data> <test-data> <lang> <ali-dir> <exp-dir>"
  echo " e.g.: steps/nnet2/get_egs3.sh data/train data/test data/lang exp/tri3_ali exp/tri4_nnet"
  echo ""
...
data=$1
testdata=$2
lang=$3  # kept for historical reasons, but never used.
alidir=$4
dir=$5
...
for f in $data/feats.scp $testdata/feats.scp $lang/L.fst \
  $alidir/ali.1.gz $alidir/final.mdl $alidir/tree \
  $extra_files; do
...
stestdata=$testdata/split$nj
utils/split_data.sh $testdata $nj
...
#awk '{print $1}' $data/utt2spk | head -$num_utts_subset  \
#    > $dir/valid_uttlist || exit 1;
#TODO
...
awk '{print $1}' $data/utt2spk > $dir/train_uttlist || exit 1;
...
case $feat_type in
  raw) feats="ark,s,cs:utils/filter_scp.pl $dir/train_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
  ...
  lda) 
    ...
    feats="ark,s,cs:utils/filter_scp.pl $dir/train_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    valid_feats="ark,s,cs:utils/filter_scp.pl $dir/valid_uttlist $testdata/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$testdata/utt2spk scp:$testdata/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
...
if [ -f $transform_dir/trans.1 ] && [ $feat_type != "raw" ]; then
  ...
  valid_feats="$valid_feats transform-feats --utt2spk=ark:$testdata/utt2spk 'ark:cat $transform_dir/trans.*|' ark:- ark:- |"
...
if [ -f $transform_dir/raw_trans.1 ] && [ $feat_type == "raw" ]; then
  ...
  valid_feats="$valid_feats transform-feats --utt2spk=ark:$testdata/utt2spk 'ark:cat $transform_dir/raw_trans.*|' ark:- ark:- |"
