...
num_threads_ubm=1
nnet3_affix=_wosp       # affix for exp dirs, e.g. it was _cleaned in tedlium.
...
progress/nnet3/run_ivector_common_wosp.sh \
  --stage $stage --nj $nj \
...
dir=exp/nnet3${nnet3_affix}/tdnn_lstm/epoch${epoch}_tdnndim${tdnndim}_rpdim${rpdim}
train_data_dir=data/wosp/train_clean_utt_hires
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_train_clean_utt_hires

test_data_dir=data/paper/test_clean_utt_hires
test_ivector_dir=exp/nnet3/ivectors_paper/test_clean_utt_hires
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=lstm3 cell-dim=$tdnndim recurrent-projection-dim=$rpdim non-recurrent-projection-dim=$rpdim decay-time=20 delay=-3
  ...
  for data in $test_sets; do
    (
      ...
      steps/nnet3/decode.sh \
        ...
        $graph_dir $test_data_dir ${dir}/decode_tgsmall || exit 1
    ) || touch $dir/.error &
  done
  wait
