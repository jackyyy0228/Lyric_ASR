...
EOF
  ...
  dir=exp/paper/nnet_from_speech/gpu_pnorm_retrain
  trainfeats=exp/paper/nnet_from_speech/activations_vocal_train
  testfeats=exp/paper/nnet2_from_speech/activations_vocal_test
  srcdir=exp/speech/threads_pnorm_accel_clean_100
else
  ...
  dir=exp/paper/nnet_from_speech/threads_pnorm_retrain
  trainfeats=exp/paper/nnet_from_speech/activations_vocal_train
  testfeats=exp/paper/nnet2_from_speech/activations_vocal_test
  srcdir=exp/speech/threads_pnorm_accel_clean_100
fi
...
if [ $stage -le 0 ]; then
  ...
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj $numjob \
     $trainset $srcdir $trainfeats
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj $numjob \
     data/vocal/test_clean $srcdir $testfeats
fi
...
if [ $stage -le 1 ]; then
  ...
  local/nnet2/retrain_fast3.sh --stage $train_stage \
    ...
    --initial-learning-rate 0.02 --final-learning-rate 0.004 \
     $trainfeats/data $testfeats/data data/lang_lyric exp/vocal/svtri3b_svtri1_1k_5k_all_clean_ali_exp3 $dir 
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/test_clean ${dir}_online/decode || exit 1;
  wait
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
     --per-utt true \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/test_clean ${dir}_online/decode_utt || exit 1;
  wait
fi
...
if [ $stage -le 5 ]; then
  ...
  #initial_learning_rate=0.01#in a, b
...
if [ $stage -le 6 ]; then
  ...
  local/online/nnet2/get_egs3.sh --cmd "$train_cmd" --num-jobs-nnet 4 \
    data/vocal/ori_train_clean data/vocal/test_clean exp/vocal/svtri3b_svtri1_1k_5k_all_clean_ali_exp3 ${dir}_online ${dir}_combined
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/test_clean ${dir}_combined_online/decode &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    --per-utt true exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/test_clean ${dir}_combined_online/decode_per_utt || exit 1;
  wait
fi
