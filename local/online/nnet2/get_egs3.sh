...
if [ $# != 5 ]; then
  echo "Usage: steps/online/nnet2/get_egs3.sh [opts] <data> <test-data> <ali-dir> <online-nnet-dir> <exp-dir>"
  echo " e.g.: steps/online/nnet2/get_egs3.sh data/train data/test exp/tri3_ali exp/nnet2_online/nnet_a_gpu_online/ exp/tri4_nnet"
...
for f in $data/wav.scp $testdata/wav.scp $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $feature_conf $mdl; do
...
awk '{print $1}' $testdata/utt2spk | utils/shuffle_list.pl \
...
cp $testdata/utt2spk $dir/valid/utt2spk || exit 1;
cp $testdata/spk2utt $dir/valid/spk2utt || exit 1;
awk '{print $1}' <$dir/valid/utt2spk >$dir/valid/uttlist_extended || exit 1;

utils/filter_scp.pl $dir/train_subset/uttlist <$data/utt2spk | awk '{print $2}' > $dir/train_subset/spklist || exit 1;
utils/filter_scp.pl -f 2 $dir/train_subset/spklist <$data/utt2spk >$dir/train_subset/utt2spk || exit 1;
utils/utt2spk_to_spk2utt.pl <$dir/train_subset/utt2spk >$dir/train_subset/spk2utt || exit 1;
awk '{print $1}' <$dir/train_subset/utt2spk >$dir/train_subset/uttlist_extended || exit 1;
rm $dir/train_subset/spklist
...
if [ -f $data/segments ]; then
  ...
  valid_feats="ark,s,cs:utils/filter_scp.pl $dir/valid/uttlist_extended $testdata/segments  | extract-segments scp:$testdata/wav.scp - ark:- | online2-wav-dump-features --config=$dir/feature.conf ark:$dir/valid/spk2utt ark,s,cs:- ark:- | subset-feats --include=$dir/valid/uttlist ark:- ark:- |"
  ...
  valid_feats="ark,s,cs:utils/filter_scp.pl $dir/valid/uttlist_extended $testdata/wav.scp | online2-wav-dump-features --config=$dir/feature.conf ark:$dir/valid/spk2utt scp:- ark:- | subset-feats --include=$dir/valid/uttlist ark:- ark:- |"
