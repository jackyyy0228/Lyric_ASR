...
if [ $stage -le 1 ]; then
  ...
  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr_pass1.JOB.log \
    ...
    ark:$dir/tmp_pre_trans.JOB || exit 1;
  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr2_pass1.JOB.log \
    gunzip -c $si_dir/lat.JOB.gz \| \
    lattice-to-post --acoustic-scale=$acwt ark:- ark:- \| \
    weight-silence-post $silence_weight $silphonelist $alignment_model ark:- ark:- \| \
    gmm-post-to-gpost $alignment_model "$sifeats" ark:- ark:- \| \
    gmm-est-fmllr-gpost --fmllr-update-type=$fmllr_update_type \
    --spk2utt=ark:$sdata/JOB/level2spk $adapt_model "$sifeats" ark,s,cs:- \
    ark,t:$dir/pre_raw_trans2.JOB || exit 1;
  for n in `seq $nj`; do
    python dup_trans.py --level2spk=$sdata/$n/level2spk --in-trans=$dir/pre_raw_trans2.$n \
      --out-trans=$dir/pre_trans2.$n
  done
  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr_compose_pass1.JOB.log \
    compose-transforms --b-is-affine=true ark:$dir/tmp_pre_trans.JOB ark,t:$dir/pre_trans2.JOB \
...
if [ $stage -le 3 ]; then
  ...
  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr_pass2.JOB.log \
    ...
    ark,s,cs:- ark:$dir/trans_tmp.JOB || exit 1;

  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/raw_fmllr_pass2.JOB.log \
    lattice-determinize-pruned$thread_string --acoustic-scale=$acwt --beam=4.0 \
    "ark:gunzip -c $dir/lat.tmp.JOB.gz|" ark:- \| \
    lattice-to-post --acoustic-scale=$acwt ark:- ark:- \| \
    weight-silence-post $silence_weight $silphonelist $adapt_model ark:- ark:- \| \
    gmm-est-fmllr --fmllr-update-type=$fmllr_update_type \
    --spk2utt=ark:$sdata/JOB/level2spk $adapt_model "$pass1feats" \
    ark,s,cs:- ark,t:$dir/raw_trans2_tmp.JOB || exit 1;

  for n in `seq $nj`; do
    python dup_trans.py --level2spk=$sdata/$n/level2spk --in-trans=$dir/raw_trans2_tmp.$n \
      --out-trans=$dir/trans2_tmp.$n
    compose-transforms --b-is-affine=true ark,t:$dir/trans2_tmp.$n ark:$dir/trans_tmp.$n \
    ark:$dir/trans_all_tmp.$n || exit 1;
  done

  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr_compose_pass2.JOB.log \
    compose-transforms --b-is-affine=true ark:$dir/trans_all_tmp.JOB ark:$dir/pre_trans.JOB \
