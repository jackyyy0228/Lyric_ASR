...
ali=exp/vocal/svtri3b_svtri1_1k_5k_ali_genre_tag_exp3
if [ $stage -le -1 ]; then
  trainset=data/vocal/genre_train_clean
  lm=data/lang_lyric
...
EOF
  ...
  dir=exp/vocal/nnet2_online/nnet_c_genre_tag
  trainfeats=exp/vocal/nnet2_online/activations_train_from_speech_to_vocal_genre_tag
...
if [ $stage -le 0 ]; then
  ...
  steps/online/nnet2/dump_nnet_activations.sh --cmd "$train_cmd" --nj 5 \
     data/vocal/genre_train_clean $srcdir $trainfeats
fi
...
if [ $stage -le 1 ]; then
  ...
  steps/nnet2/retrain_fast.sh --stage $train_stage \
    ...
    --initial-learning-rate 0.01 --final-learning-rate 0.002 \
     $trainfeats/data data/lang_lyric $ali $dir 
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/genre_test_clean ${dir}_online/decode || exit 1;
  wait
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
     --per-utt true \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/new_genre_test_clean ${dir}_online/decode_utt || exit 1;
  wait
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/get_egs.sh --cmd "$train_cmd" --num-jobs-nnet 4 \
    data/vocal/genre_train_clean $ali ${dir}_online ${dir}_combined
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/genre_test_clean ${dir}_combined_online/decode &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $numjob \
    --per-utt true exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/new_genre_test_clean ${dir}_combined_online/decode_per_utt || exit 1;
  wait
fi
