...
train_stage=-10
use_gpu=false
dir=exp/vocal/nnet2_online/nnet_ms_a
...
if [ $stage -le 7 ]; then
  ...
  steps/nnet2/train_multisplice_accel2.sh --stage $train_stage \
    ...
    --mix-up 12000 \
    data/vocal/ori_train_clean data/lang_lyric exp/vocal/svtri3b_svtri1_1k_5k_exp3 $dir  || exit 1;
fi
...
if [ $stage -le 8 ]; then
  ...
  for test in test_clean; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $numjob \
      data/vocal/${test} exp/vocal/nnet2_online/extractor exp/vocal/nnet2_online/ivectors_$test || exit 1;
  done
fi
...
if [ $stage -le 9 ]; then
  ...
  for test in test_clean; do
    steps/nnet2/decode.sh --nj $numjob --cmd "$decode_cmd" --config conf/decode.config \
      --online-ivector-dir exp/vocal/nnet2_online/ivectors_${test} \
      exp/vocal/svtri3b_svtri1_1k_5k_exp3/graph_tgsmall data/vocal/${test} $dir/decode_${test}_tgsmall || exit 1;
  done
  wait
fi
