...
dev=data-fbank/vocal/test_clean
train=data-fbank/vocal/train_clean

dev_original=data/vocal/test_clean
train_original=data/vocal/ori_train_clean

gmm=exp/vocal/svtri3b_svtri1_1k_5k_exp3
...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train.sh \
      ...
      ${train}_tr90 ${train}_cv10 data/lang_lyric $ali $ali $dir || exit 1;
  # Decode,
...
if [ $stage -le 3 ]; then
  dir=exp/vocal/cnn4c_pretrain-dbn
  transf_cnn=exp/vocal/cnn4c/final.feature_transform_cnn # transform with convolutional layers
  # Train
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/align.sh --nj 10 --cmd "$train_cmd" \
    $train data/lang_lyric $dir ${dir}_ali || exit 1
fi

# Train the DNN optimizing cross-entropy,
if [ $stage -le 5 ]; then
  dir=exp/vocal/cnn4c_pretrain-dbn_dnn; [ ! -d $dir ] && mkdir -p $dir/log;
  ali=exp/vocal/cnn4c_ali
  feature_transform=exp/vocal/cnn4c/final.feature_transform
  feature_transform_dbn=exp/vocal/cnn4c_pretrain-dbn/final.feature_transform
  dbn=exp/vocal/cnn4c_pretrain-dbn/4.dbn
...
# Note: With DNNs in RM, the optimal LMWT is 2-6. Don't be tempted to try acwt's like 0.2, 
# the value 0.1 is better both for decoding and sMBR.
dir=exp/vocal/cnn4c_pretrain-dbn_dnn_smbr
srcdir=exp/vocal/cnn4c_pretrain-dbn_dnn
acwt=0.1

# First we generate lattices and alignments,
if [ $stage -le 6 ]; then
  steps/nnet/align.sh --nj 10 --cmd "$train_cmd" \
    $train data/lang_lyric $srcdir ${srcdir}_ali || exit 1;
  steps/nnet/make_denlats.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
    $train data/lang_lyric $srcdir ${srcdir}_denlats || exit 1;
fi

# Re-train the DNN by 6 iterations of sMBR,
if [ $stage -le 7 ]; then
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 6 --acwt $acwt --do-smbr true \
    $train data/lang_lyric $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode
