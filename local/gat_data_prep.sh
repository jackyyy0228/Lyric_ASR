...
for spk_dir in $(find $src -mindepth 1 -maxdepth 1 -type d | sort); do
  ...
  for song_dir in $(find -L $spk_dir/ -mindepth 1 -maxdepth 1 -type d | sort); do
    ...
    awk -v "speaker=$speaker" -v "song=$song" '{printf "%s %s-%s\n", $1, speaker, song}' \
      <$song_trans >>$utt2spk || exit 1
    awk -v "genre=$genre" '{printf "%s %s\n", $1, genre}' \
      <$song_trans >>$utt2genre || exit 1
  done
done

spk2utt=$dst/spk2utt
old_steps_and_utils/utils/utt2spk_to_spk2utt.pl <$utt2spk >$spk2utt || exit 1
genre2utt=$dst/genre2utt
old_steps_and_utils/utils/utt2spk_to_spk2utt.pl <$utt2genre >$genre2utt || exit 1
