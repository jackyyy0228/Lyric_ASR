...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <testdata> <lang> <ali-dir> <exp-dir>"
  echo " e.g.: $0 data/train data/test data/lang exp/tri3_ali exp/tri4_nnet"
  echo ""
...
# Check some files.
for f in $data/feats.scp $testdata/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree; do
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  echo "$0: calling get_egs3.sh"
  local/nnet2/get_egs3.sh --feat-type raw --cmvn-opts "--norm-means=false --norm-vars=false" \
      ...
      $data $testdata $lang $alidir $dir || exit 1;
fi
