...
EOF
  ...
  minibatch_size=128
  egs_dir=exp/paper/nnet_fbank/egs
  dir=exp/paper/nnet_fbank/layer${num_layer}_$1\_$2\_epoch${epoch}_melbin_40_splice${splice}_lr0.015
fi
...
if [ !  -f $dir/final.mdl ]; then
   ...
   local/nnet2/train_pnorm_fast_fbank.sh --stage $train_stage \
   --samples-per-iter 200000 \
   --parallel-opts "$parallel_opts" \
   --minibatch-size "$minibatch_size" \
   --num-jobs-nnet 8 \
   --initial-learning-rate 0.015 --final-learning-rate 0.0015 \
